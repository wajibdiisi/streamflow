...
app.post('/stop-stream', async (req, res) => {
  ...
  console.log('Menghentikan stream:', stream_key);
    ...
    try {
      ...
      try {
        await new Promise((resolve, reject) => {
          database.updateStreamContainer(containerId, { is_streaming: 0 }, (err) => {
            if (err) {
              console.error('Error updating stream container:', err);
              return reject(new Error('Gagal memperbarui status stream di database:' + err.message));
            }
            deleteFile(videoPath);
            console.log(`Database updated after ${Date.now() - startTime}ms`);
            resolve();
          });
        });
...
app.get('/stream-status/:streamKey', (req, res) => {
  ...
  res.on('close', () => {
    clearInterval(intervalId);
    console.log('Client disconnected from stream status:', streamKey);
    if (streams[streamKey]) {
