...
async function buildFFmpegCommand(stream, seekSeconds) {
  ...
  console.log(`[StreamingService] Seek seconds: ${effectiveSeek}`);
  ...
  console.log(`[StreamingService] Command type: ${typeof command}`);
  console.log(`[StreamingService] Has on method: ${typeof command.on}`);
...
async function stopStream(streamId) {
        ...
        if (wasActive) {
          try {
            ...
            try {
              ...
              const sessionStart = sessionStartTime ? new Date(sessionStartTime) : null;
              const runtimeMinutes = sessionStart ? Math.ceil((Date.now() - sessionStartTime)/60000) : null;
              telegram.notifyStreamStop(stream.user_id, streamId, { runtimeMinutes }).catch(()=>{});
            } catch (e) {}
          } catch (error) {
      ...
      if (signal === 'SIGSEGV') {
        ...
        if (retryCount < MAX_RETRY_ATTEMPTS && allowRestart) {
          ...
          try {
            const telegram = require('./telegramService');
            telegram.notifyStreamError(stream.user_id, streamId, { signal: 'SIGSEGV' }).catch(()=>{});
          } catch (e) {}
          setTimeout(async () => {
            try {
        ...
        if (code !== 0 && code !== null) {
          ...
          if (currentStream && typeof currentStream.duration === 'number' && currentStream.duration <= 0) {
            ...
            try {
              ...
              try {
                const telegram = require('./telegramService');
                telegram.notifyStreamError(stream.user_id, streamId, { code }).catch(()=>{});
              } catch (e) {}
            } catch (error) {
              console.error(`[StreamingService] Error updating stream status after remaining reached zero: ${error.message}`);
            }
          ...
          if (isRecoverableError) {
            ...
            if (retryCount < MAX_RETRY_ATTEMPTS && allowRestart) {
              ...
              try {
                const telegram = require('./telegramService');
                telegram.notifyStreamError(stream.user_id, streamId, { code, message: trackedError || restartType }).catch(()=>{});
              } catch (e) {}
        ...
        if (wasActive) {
          try {
            ...
            try {
              const telegram = require('./telegramService');
              // Calculate session runtime in minutes
              const prevStart = sessionStartTime ? new Date(sessionStartTime) : null;
              const runtimeMinutes = prevStart ? Math.ceil((Date.now() - sessionStartTime)/60000) : null;
              telegram.notifyStreamStop(stream.user_id, streamId, { runtimeMinutes }).catch(()=>{});
            } catch (e) {}
          } catch (error) {
